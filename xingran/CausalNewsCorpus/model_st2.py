...
from transformers import AutoModel, AutoTokenizer, AutoConfig, AutoModelForSequenceClassification, AutoModelForMaskedLM
...
class ST2ModelV2(nn.Module):
    ...
    #     start_effect = None
    #     end_effect = None
    #     for i in range(len(start_cause_logits)):
    #         if attention_mask[i] == 0:
    #             break
    #         for j in range(len(end_cause_logits)):
    #             if attention_mask[j] == 0:
    #                 break
                
    #             for m in range(i):
    #                 for n in range(m, i):
    #                     score = start_cause_logits[i] + end_cause_logits[j] + start_effect_logits[m] + end_effect_logits[n]
    #                     if score > best_score:
    #                         best_score = score
    #                         start_cause, end_cause, start_effect, end_effect = i, j, m, n              

    #             for m in range(j, len(start_effect_logits)):
    #                 if attention_mask[m] == 0:
    #                     break
    #                 for n in range(m, len(end_effect_logits)):
    #                     if attention_mask[n] == 0:
    #                         break
    #                     score = start_cause_logits[i] + end_cause_logits[j] + start_effect_logits[m] + end_effect_logits[n]
    #                     if score > best_score:
    #                         best_score = score
    ...
    #     assert start_cause is not None
    #     assert end_cause is not None
    #     assert start_effect is not None
